...
describe MembershipMailer do
  ...
  describe '.dailyreading_email' do
    let(:user){create(:user)}
    let(:challenge){create(:challenge)}
    let!(:membership){challenge.join_new_member(user)}

    it "sends an email to a challenge member" do
      expect{ MembershipMailer.dailyreading_email(membership, challenge.readings.first).deliver }.to_not raise_error
    end
