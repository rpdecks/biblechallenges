...
feature 'User logs reading via email' do
  ...
  scenario 'When a user logs his reading, he sees the comments for each reading in the confirmation page' do
    user = create(:user)
    challenge = create(:challenge, :with_readings, chapters_to_read: "Mat 1-2", num_chapters_per_day: 1)
    create(:membership, user: user, challenge: challenge)
    ...
    ReadingMailer.daily_reading_email([reading.id], user.id).deliver_now
    open_last_email
    visit_in_email("Confirm")
